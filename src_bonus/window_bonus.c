...
void	check_texture(t_map **map, int y, int x)
{
	int	size;
...
void	init_window(t_map **map)
{
	...
	(*map)->mlx_window = mlx_new_window((*map)->mlx_ptr,
			(*map)->width * TEXTURE, (*map)->height * TEXTURE + 32,
			"so_long");
	mlx_string_put((*map)->mlx_ptr, (*map)->mlx_window, 20,
		(*map)->height * TEXTURE + 20, 0xd7e4fa, "Movements: ");
	if (!(*map)->mlx_window)
		clean_and_exit("Error with malloc", map);
	draw(map);
	...
	mlx_loop_hook((*map)->mlx_ptr, anim, map);
